var Timeline = function(container, date) {
    ...
    this.weekDays     = ["Domingo","Lunes","Martes","MiÃ©rcoles","Jueves","Viernes","SÃ¡bado"];
    ...
    this.renderDay = function(topRow, bottomRow, data){
		...
		var cssClass = "today";
		  ...
		  topRow.append("<li><div id=\"d_"+data.name+"\" class=\"" + cssClass + "\"  onclick='callTheJsonp(\"14/11/2012\");("+data.number+")' )'>" + data.name   + "</div></li>");

        // OBTENGO LAS FECHAS QUE SE ESTÃN GENERANDO Y LAS DE HOY
        var temporal = new Date((this.date.getMonth()+1) + "/" + data.number + "/" + this.date.getFullYear());
        var todayDate = new Date();
		console.log(data.number + " Date");
        // COMPARO LAS FECHAS EN FORMATO UNIVERSAL Y SI LA ACTUAL ES MENOR, SE DIBUJARÃ EL DÃA DESACTIVADO
        if(todayDate.getTime() < temporal.getTime()){
            //console.log((temporal.getDate() + "/" + (temporal.getMonth()+1) + "/" + temporal.getFullYear()) + " ES MAYOR");
            bottomRow.append("<li><div id=\"d_"+data.number+"\" class=\"inactive \">" + data.number + "</div></li>");
        }else
            bottomRow.append("<li><div id=\"d_"+data.number+"\" class=\"active " + cssClass + "\" onclick=\"changeDay('"+data.number+"/"+
			(this.date.getMonth()+1)+"/"+this.date.getFullYear()+"')\" onmouseover='calendarOver(this,\""+data.number+"/"+
			(this.date.getMonth()+1)+"/"+this.date.getFullYear()+"\")'  >" + data.number + "</div></li>");
